...
const QuestionForm = dynamic(() => import('@/components/aptitude/QuestionForm'), {
  ssr: false
});
...
export default function Home() {
  ...
  const handleQuestionsGenerated = (newQuestions, generatedTopic) => {
    ...
    // Store questions exactly as received
  ...
  const submitQuiz = async () => {
    ...
    try {
      ...
      setQuizStarted(false); // Allow viewing questions after results
    } catch (err) {
  ...
  useEffect(() => {
    if (hasQuestions && !quizStarted && !loading && !quizResult) {
      startQuiz();
    }
  }, [hasQuestions, quizStarted, loading, quizResult]);
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-xl p-6">
            ...
            {/* Quiz Interface */}
            {!loading && hasQuestions && quizStarted && !quizResult && (
              <div>
                ...
                <div className="mt-6 flex justify-between">
                  ...
                  <button
                    onClick={submitQuiz}
                    className={`inline-flex items-center px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 ${submittingAnswers ? 'opacity-50 cursor-not-allowed' : ''}`}
                    ...
                    <div key={qIndex} className="border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      <div className="p-4">
                        ...
                        <div className="space-y-2">
                          {Array.isArray(q.options) && q.options.length > 0 ? (
                            q.options.map((option, oIndex) => {
                              const isCorrect = determineCorrectOption(q, option);
                              const letter = String.fromCharCode(65 + oIndex);
                              return (
                                <div
                                  key={oIndex}
            ...
            {!loading && !hasQuestions && !quizResult && (
              <div className="text-center py-8">
                <p className="text-gray-500">No questions available. Please generate new questions.</p>
                <button
